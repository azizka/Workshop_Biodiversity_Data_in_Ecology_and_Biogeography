...
1. Load the example data (we will use "lion_gbif.csv"), and limit the data to columns with potentially useful information (`read_csv`, `names`, `select`).
...
4. Apply the automated cleaning as implemented in speciesgeocodeR, to remove problematic records.
5. Apply the automated dataset level cleaning as implemented in speciesgeocodeR to see if there are potential problems with coordinate precision. Try this on the Bombacoideae example data as well (bombacoideae_gbif_occurrences.csv).
